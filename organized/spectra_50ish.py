...
for i in np.arange(len(split)-1):
   ...
   dat_xy = u_xy[split[i]:split[i+1],0]
   dat_z = u_z[split[i]:split[i+1],0]
   dat_u = u_u[split[i]:split[i+1],0]
...
for i in np.arange(len(split_t)-1):
   ...
   dat = u_t[split_t[i]:split_t[i+1],0]
...
plt.savefig('spectra_50ish.png',dpi=600)
plt.show()
