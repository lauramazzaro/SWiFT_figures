...
t_xy = np.append(t_xy,dh['t_xy'][18001:],axis=0)
t_z = np.append(t_z,dh['t_z'][18001:],axis=0)
t_u = np.append(t_u,dh['t_u'][18001:],axis=0)
t_t = np.append(t_t,dh['t_t'][90001:],axis=0)
u_xy = np.append(u_xy,dh['u_xy'][18001:,:],axis=0)
u_z = np.append(u_z,dh['u_z'][18001:,:],axis=0)
u_u = np.append(u_u,dh['u_u'][18001:,:],axis=0)
u_t = np.append(u_t,dh['u_t'][90001:,:],axis=0)
v_xy = np.append(v_xy,dh['v_xy'][18001:,:],axis=0)
v_z = np.append(v_z,dh['v_z'][18001:,:],axis=0)
v_u = np.append(v_u,dh['v_u'][18001:,:],axis=0)
v_t = np.append(v_t,dh['v_t'][90001:,:],axis=0)
U_t = ((u_t)**2+(v_t)**2)**(1/2.)
U_u = ((u_u)**2+(v_u)**2)**(1/2.)
U_z = ((u_z)**2+(v_z)**2)**(1/2.)
...
# Variable data:
dh = np.load('var_data.npz')
t_var = dh['t']
u_var = dh['u'][:,[4,10]]
v_var = dh['v'][:,[4,10]]
U_var = ((u_var)**2+(v_var)**2)**(1/2.)

t_var = np.array([datetime.datetime(2013,11,8,int(t_var[i])+12,int((t_var[i]*60)%60),int((t_var[i]*60*60)%60),int((t_var[i]*60*60*10)%10)) for i in np.arange(len(t_var))])
...
ave_ind_var = np.arange(0,len(t_var)+10*60/dt,10*60/dt)
ave_ind_var = ave_ind_var.tolist()

xmin = datetime.datetime(2013,11,8,20,5,0,0)
xmax = datetime.datetime(2013,11,8,22,55,0,0)

U_xy_ave = np.zeros([len(ave_ind)-1,2])
U_z_ave = np.zeros([len(ave_ind)-1,2])
U_u_ave = np.zeros([len(ave_ind)-1,2])
U_var_ave = np.zeros([len(ave_ind_var)-1,2])
...
U_var_std_1 = np.zeros([len(ave_ind_var)-1,2])
...
U_var_std_2 = np.zeros([len(ave_ind_var)-1,2])
...
for i in np.arange(len(ave_ind_var)-1):

   i1 = ave_ind_var[i]
   i2 = ave_ind_var[i+1]

   U_var_ave[i,:] = np.squeeze(np.nanmean(U_var[i1:i2,:],0))

   U_var_std  = (np.nanmean((U_var[i1:i2,:]-U_var_ave[i,:])**2))**(.5)

   U_var_std_1[i,:] = U_var_ave[i,:] - U_var_std
   U_var_std_2[i,:] = U_var_ave[i,:] + U_var_std
...
t_var = t_var[ave_ind_var[:-1]]
t_var = [t_var[i]+datetime.timedelta(minutes = 5) for i in np.arange(len(ave_ind_var)-1)]
t_t = t_t[ave_ind_t[:-1]]
...
plt.plot(t_var,U_var_ave[:,1],c=line_cols[3],linewidth=2)
...
plt.fill_between(t_var,U_var_std_1[:,1],U_var_std_2[:,1],alpha=0.3,color=line_cols[3])
...
# new axis:
ax2 = plt.gca().twiny()
mn, mx = [xmin-datetime.timedelta(hours=-6),xmax-datetime.timedelta(hours=-6)]
ax2.set_xlim(mn,mx)
...
plt.plot(t_var,U_var_ave[:,0],c=line_cols[3],linewidth=2)
plt.fill_between(t_var,U_var_std_1[:,0],U_var_std_2[:,0],alpha=0.3,color=line_cols[3])
...
plt.text(0.6,0.9,R'F$_\mathrm{xy+z}$',color=line_cols[3],transform=plt.gca().transAxes)
plt.text(0.7,0.9,R'TTUT',color='black',transform=plt.gca().transAxes)
...
plt.savefig('var_mean.png',dpi=600)
plt.show()
