...
ave_ind = np.arange(0,len(t_xy),10*60/dt)
ave_ind = ave_ind.tolist()
ave_ind_t = np.arange(0,len(t_t),10*60/dt_t)
ave_ind_t = ave_ind_t.tolist()

xmin = datetime.datetime(2013,11,8,21,35,0,0)
...
plt.fill_between(t_u,U_t_std_1[:,1],U_t_std_2[:,1],alpha=0.3,color='k')
...
plt.savefig('n_mean.png',dpi=600)
plt.show()
