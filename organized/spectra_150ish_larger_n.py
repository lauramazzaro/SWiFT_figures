...
dh = np.load('u_larger_n.npz')
u_u_2 = dh['u']
v_u_2 = dh['v']

dh = np.load('xy_larger_n.npz')
u_xy_2 = dh['u']
v_xy_2 = dh['v']

dh = np.load('z_larger_n.npz')
u_z_2 = dh['u']
v_z_2 = dh['v']
...
Ef_xy_2 = [None]*(len(split)-4)
Ef_z_2 = [None]*(len(split)-4)
Ef_u_2 = [None]*(len(split)-4)
...
for i in np.arange(len(split)-4):
...
plt.savefig('spectra_150ish_larger_n.png',dpi=600)
plt.show()
