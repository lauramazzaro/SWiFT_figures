...
ave_ind = np.arange(0,len(t_z)+10*60/dt,10*60/dt)
ave_ind = ave_ind.tolist()
ave_ind_t = np.arange(0,len(t_t)+10*60/dt_t-1,10*60/dt_t)
ave_ind_t = ave_ind_t.tolist()

xmin = datetime.datetime(2013,11,9,04,5,0,0)
xmax = datetime.datetime(2013,11,9,04,55,0,0)
...
#plt.plot(t_xy,U_xy_ave[:,1],c=line_cols[1],linewidth=2)
...
plt.fill_between(t_u,U_t_std_1[:,1],U_t_std_2[:,1],alpha=0.3,color=line_cols[-1])
...
plt.fill_between(t_u,U_t_std_1[:,0],U_t_std_2[:,0],alpha=0.3,color=line_cols[-1])
...
plt.savefig('s_mean.png',dpi=600)
plt.show()
